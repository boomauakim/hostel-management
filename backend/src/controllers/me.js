...
const getBooking = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const first = await BookingModel.findOne({ user: uid }).sort({ id: 1 }).limit(1);
      const last = await BookingModel.findOne({ user: uid }).sort({ id: -1 }).limit(1);
      let next = '';
      let previous = '';
      ...
      if (data.length > 0 && data[data.length - 1] && !last._id.equals(data[data.length - 1]._id)) {
        next = `${host}/api/me/bookings?after=${data[data.length - 1]._id}&limit=${query.limit}`;
      }
